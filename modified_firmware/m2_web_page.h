...
<body>
    <main>
        <div>
			...
			<div>
			    <button class="btn-of btn-main-m btn-all-bg" onclick="window.location.href='/servoConfig'">SERVO ID CONFIG</button>
			    <button class="btn-of btn-main-m btn-all-bg" onclick="cmdInit();">INIT POS</button>
			</div>
                    </div>
                    <div>
...
const char servo_config_html[] PROGMEM = R"rawliteral(
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Servo ID Configuration</title>
    <style type="text/css">
        body {
            background-image: linear-gradient(#3F424F, #1E212E);
        ...
        .btn:hover { background-color: rgba(164,169,186,0.15); }
        .btn-scan {
            background-color: rgba(38,152,234,0.3);
            color: rgba(255,255,255,0.8);
            width: 100%;
            font-size: 16px;
            margin: 10px 0;
        }
        .btn-scan:hover { background-color: rgba(38,152,234,0.5); }
        .btn-change {
            background-color: rgba(76,175,80,0.3);
            color: rgba(255,255,255,0.8);
        }
        .btn-change:hover { background-color: rgba(76,175,80,0.5); }
        .servo-list {
            margin: 20px 0;
        }
        .servo-item {
        ...
        .servo-item .servo-info {
            flex: 1;
        }
        .servo-item .servo-info p {
            margin: 3px 0;
        }
        .servo-item .servo-actions {
            display: flex;
            align-items: center;
            gap: 10px;
        }
        .servo-item input[type="number"] {
            width: 60px;
            height: 36px;
            background-color: rgba(0,0,0,0.3);
            border: 1px solid rgba(194,196,201,0.3);
            border-radius: 4px;
            color: rgba(255,255,255,0.8);
            font-size: 16px;
            text-align: center;
        ...
        @media screen and (max-width: 767px) {
            main { width: 94vw; }
            .servo-item { flex-direction: column; gap: 10px; }
            .servo-item .servo-actions { width: 100%; justify-content: flex-end; }
        }
    </style>
</head>
<body>
    <main>
        <div class="headbar">
            <div class="text_btn" onclick="window.location.href='/'">&lt; MAIN PAGE</div>
        </div>
        <h2>Servo ID Configuration</h2>

        <div class="known-ids">
            <p>Known RoArm-M2 servo IDs: <span>11</span> (Base), <span>12</span> (Shoulder Drive), <span>13</span> (Shoulder Driven), <span>14</span> (Elbow), <span>15</span> (Gripper)</p>
            <p class="warning">⚠ Only connect ONE servo when changing IDs to avoid conflicts. Make sure 12V power is connected.</p>
        </div>

        <button class="btn btn-scan" onclick="scanServos()">🔍 SCAN CONNECTED SERVOS</button>
    ...
    <script>
        ...
        function scanServos() {
            const listEl = document.getElementById('servoList');
            listEl.innerHTML = '<div class="status-msg status-info"><span class="spinner"></span>Scanning servos (ID 0-30)... please wait.</div>';
            clearStatus();
            ...
            xhr.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    try {
                        ...
                        displayServos(response);
                    } catch(e) {
                        listEl.innerHTML = '';
                        showStatus('Error parsing response: ' + this.responseText, 'error');
                    }
                } else if (this.readyState == 4) {
                    listEl.innerHTML = '';
                    showStatus('Communication error. Is the device connected?', 'error');
                }
            };
            xhr.timeout = 15000;
            xhr.ontimeout = function() {
                listEl.innerHTML = '';
                showStatus('Scan timed out. The device may be busy.', 'error');
            };
            xhr.open("GET", "js?json=" + jsonString, true);
            xhr.send();
        }

        function displayServos(response) {
            const listEl = document.getElementById('servoList');
            listEl.innerHTML = '';

            var count = response.count || 0;
            if (count === 0) {
                showStatus('No servos found. Check 12V power and cable connections.', 'error');
                return;
            }

            showStatus('Found ' + count + ' servo(s).', 'success');

            for (var i = 0; i < count; i++) {
                var servo = response['servo_' + i];
                if (!servo) continue;

                var id = servo.id;
                var pos = servo.pos;
                var knownName = KNOWN_SERVOS[id] || "Unknown";

                var itemHtml = '<div class="servo-item" id="servo-row-' + id + '">';
                itemHtml += '<div class="servo-info">';
                itemHtml += '<p><strong>ID: ' + id + '</strong> (' + knownName + ')</p>';
                itemHtml += '<p style="font-size:12px;color:rgba(255,255,255,0.4);">Position: ' + pos + '</p>';
                itemHtml += '</div>';
                itemHtml += '<div class="servo-actions">';
                itemHtml += '<label style="font-size:12px;">New ID:</label>';
                itemHtml += '<input type="number" id="newid-' + id + '" min="0" max="253" value="' + id + '">';
                itemHtml += '<button class="btn btn-change" onclick="changeServoId(' + id + ')">CHANGE</button>';
                itemHtml += '</div>';
                itemHtml += '</div>';
        ...
        function changeServoId(oldId) {
            var newIdInput = document.getElementById('newid-' + oldId);
            var newId = parseInt(newIdInput.value);

            if (isNaN(newId) || newId < 0 || newId > 253) {
                showStatus('Invalid new ID. Must be between 0 and 253.', 'error');
                return;
            }

            if (newId === oldId) {
                showStatus('New ID is the same as current ID (' + oldId + '). No change needed.', 'info');
                return;
            }

            if (!confirm('Change servo ID from ' + oldId + ' to ' + newId + '?\n\nMake sure only ONE servo is connected!')) {
                return;
            }

            showStatus('<span class="spinner"></span>Changing ID from ' + oldId + ' to ' + newId + '...', 'info');

            var jsonCmd = {"T": 501, "raw": oldId, "new": newId};
            var jsonString = JSON.stringify(jsonCmd);
            var xhr = new XMLHttpRequest();
            xhr.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    showStatus('ID change command sent (ID ' + oldId + ' → ' + newId + '). Rescanning to verify...', 'success');
                    // Wait a moment then rescan to verify
                    setTimeout(function() {
                        scanServos();
                    }, 1500);
                } else if (this.readyState == 4) {
                    showStatus('Error sending change command.', 'error');
                }
            };
            xhr.open("GET", "js?json=" + jsonString, true);
            xhr.send();
