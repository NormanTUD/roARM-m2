...
// Scan for all connected servos across ID range 0-30
// Returns found servo IDs as JSON
void scanConnectedServos() {
  jsonInfoHttp.clear();
  jsonInfoHttp["T"] = 504;
  int foundCount = 0;
  ...
  for (int id = 0; id <= 30; id++) {
    ...
    if (pos != -1) {
      String key = "servo_" + String(foundCount);
      jsonInfoHttp[key]["id"] = id;
      jsonInfoHttp[key]["pos"] = pos;
      foundCount++;
      if (InfoPrint == 1) {
        Serial.printf("Found servo ID: %d, pos: %d\n", id, pos);
      }
    }
    delay(5);
  }

  jsonInfoHttp["count"] = foundCount;

  if (foundCount == 0) {
    jsonInfoHttp["info"] = "No servos found";
    if (InfoPrint == 1) {
      Serial.println("No servos found.");
    }
  } else {
    jsonInfoHttp["info"] = "Scan complete";
    if (InfoPrint == 1) {
      Serial.printf("Scan complete. Found %d servo(s).\n", foundCount);
    }
  }
}
